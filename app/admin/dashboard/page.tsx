...
import { 
  ...
  Trash2, Check, X, Clock, ExternalLink, Menu, ChevronRight
} from 'lucide-react';
import { useRouter } from 'next/navigation';
// Fix: Added missing motion and AnimatePresence imports from framer-motion
      ...
      <nav className="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 px-6 py-3 flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => router.push('/')} className="p-2.5 bg-slate-50 hover:bg-blue-50 text-slate-400 hover:text-blue-600 rounded-xl transition-all border border-slate-200">
            ...
            {/* Mobile Nav Trigger */}
            <button onClick={() => setIsMobileNavOpen(!isMobileNavOpen)} className="lg:hidden p-2.5 bg-slate-100 text-slate-600 rounded-xl">
      ...
      <AnimatePresence>
        {isMobileNavOpen && (
          <div className="fixed inset-0 z-[60] lg:hidden">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setIsMobileNavOpen(false)} />
            <motion.div 
              initial={{ y: -100, opacity: 0 }} 
              animate={{ y: 0, opacity: 1 }} 
              exit={{ y: -100, opacity: 0 }}
              className="absolute top-16 left-0 w-full bg-white border-b border-slate-200 p-6 space-y-4 shadow-xl"
            >
              <div className="grid grid-cols-2 gap-3">
                 <button onClick={() => { setActiveTab('ORDERS'); setIsMobileNavOpen(false); }} className={`p-5 rounded-2xl font-black italic uppercase text-xs border-2 transition-all flex items-center justify-center gap-2 ${activeTab === 'ORDERS' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-400 border-slate-100'}`}>
                   <LayoutDashboard className="w-4 h-4" /> Orders
                 </button>
                 <button onClick={() => { setActiveTab('SUPPORT'); setIsMobileNavOpen(false); }} className={`p-5 rounded-2xl font-black italic uppercase text-xs border-2 transition-all flex items-center justify-center gap-2 ${activeTab === 'SUPPORT' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-400 border-slate-100'}`}>
                   <Mail className="w-4 h-4" /> Support
                 </button>
              </div>
              <div className="pt-4 border-t border-slate-100 text-center">
                <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">Protocol Management Mode</p>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        {activeTab === 'ORDERS' ? (
          <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              ...
              <div className="flex gap-2 bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto no-scrollbar">
                {(['ALL', 'FREE', 'PREMIUM'] as const).map((t) => (
                  <button key={t} onClick={() => setFilter(t)} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase transition-all whitespace-nowrap ${filter === t ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-400 hover:bg-slate-50'}`}>{t}</button>
                ))}
              </div>
            </div>
            ...
            {loading ? (
               <div className="py-32 flex flex-col items-center justify-center gap-4">
                 ...
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-[4px]">Syncing Database...</p>
               </div>
            ) : (
              <div className="grid gap-4">
                     ...
                     <div className="flex items-center gap-8 mt-6 md:mt-0">
                        ...
                        <div className="flex flex-col items-end gap-2 min-w-[160px]">
                           ...
                           <div className="flex items-center gap-1.5 mt-2">
                             ...
                             <button onClick={() => updateOrderStatus(order.id, 'waiting_admin')} title="Pending" className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-all border-blue-200">
                ...
                {orders.length === 0 && (
                  <div className="py-32 bg-white rounded-[32px] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-4">
                    <p className="text-slate-300 font-black text-xs uppercase tracking-[5px] italic">Antrean Kosong</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                <h2 className="text-3xl font-black uppercase tracking-tight italic">Support Inbox</h2>
                <div className="flex items-center gap-2 mt-1">
            ...
            <div className="grid gap-6">
              {supportLogs.map((log) => (
                <div key={log.id} className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm space-y-6 hover:border-blue-300 hover:shadow-2xl hover:shadow-blue-50/30 transition-all duration-500 relative overflow-hidden">
              ...
              {supportLogs.length === 0 && (
                <div className="py-32 text-center text-slate-300 font-black text-[10px] uppercase tracking-[8px] italic">Inbox Kosong</div>
              )}
            </div>
          </div>
        )}
